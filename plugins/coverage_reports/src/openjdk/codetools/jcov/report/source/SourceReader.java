...
public class SourceReader {
    ...
    public SourceReader(SourceHierarchy source, SourceFilter filter) {
        this.source = source;
        this.filter = filter;
    }

    public List<String> read(String file) throws IOException {
        var lines = Files.readAllLines(source.resolve(file));
        return filter.ranges(file).stream().flatMap(lr -> lines.subList(lr.first() -1 , lr.last()).stream())
